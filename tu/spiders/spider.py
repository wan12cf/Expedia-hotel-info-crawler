...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        crawltimes = 1
        #checkInDateInput = ""
        checkInDateInput = "05/05/2017"
        #checkOutDateInput = "04/23/2017"       
        ...
        def mainwinSearch (city,checkIn, checkOut, one):
            # get the start window handle
            mwin = self.driver.current_window_handle
            ...
            dest.send_keys(city)
            checkInDate.clear()
            checkInDate.send_keys(checkIn)
            checkOutDate.clear()
            checkOutDate.send_keys(checkOut)
            if (int(one) == 1):
                people.click()
            time.sleep(2)
        ...
        def switchwin(mainWinHandle):
            ...
            for win in allwin:
                if win != mainWinHandle:
                    ...
                    self.driver.switch_to_window(win)
                    self.driver.close()
                    print win," main win is closed."
            # get back to new window and prepare to work        
            self.driver.switch_to_window(newwin)
            print "backed to the new window"
            #time.sleep(4)

        # return a string representing of the date
        def dateStr(date):
            strdate = date.strftime('%m/%d/%Y')
            return strdate

        # return a date object that is dayone + timeinterval
        def dateGen(dayone, interval):
            tomm = dayone + datetime.timedelta(days=int(interval))
            return tomm
        ...
        if(checkInDateInput !=''):
            today = strDate(checkInDateInput)
        else:
            today = datetime.date.today()

        crawlcounter = 0
        while (crawlcounter < crawltimes):
            crawlcounter += 1

            checkInDateInput = dateStr(today)
            today = dateGen(today, timeinterval)
            checkOutDateInput = dateStr(today)

            element = WebDriverWait(self.driver, 30).until(
                EC.presence_of_element_located((By.ID, 'tab-hotel-tab-hp'))
            )

            mainwin = mainwinSearch(destInput,checkInDateInput, checkOutDateInput, oneP)
            ...
            # get result number,page number by extract the value attributes of the tag <nav>
            ...
            for pagenum in range(1, (totalpage+1)):
                ...
                def waitresultload(firstNum):
                    sel = scrapy.Selector(text = self.driver.page_source)            
                    # make sure the page load is completed by checking "nav/@data-first-result" value
                    resultwait = 30
                    while (sel.xpath(".//*[@id='paginationContainer']/nav/@data-first-result").extract_first() != str(firstNum)):                    
                        ...
                        if (resultwait == 30):
                            ...
                            self.driver.refresh()
                        if (resultwait == 60):
                            print "load failed again, current pagenum is: ", pagenum
                            print "quit driver"
                            self.driver.quit()
                        sel = scrapy.Selector(text = self.driver.page_source)
                            ...
                            if(win != mainwin):
                                ...
                                for r in rooms:
                                    ...
                                    for rp in roomprice:
                                        ...
                                        hotelmodel["price"] = stripstr(rp)[1:]
                                        ...
                                        hotelmodel["rates"] = stripstr(rates)
